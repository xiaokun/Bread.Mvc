...
public class AutoGrid : Grid
{
    ...
    /// </summary>
    static void OnSpacingChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
    {
        var grid = d as AutoGrid;
        if (grid == null) return;

        foreach (UIElement child in grid.Children) {
            if (grid.Spacing.HasValue)
                child.SetValue(FrameworkElement.MarginProperty, grid.Spacing);
            else
    ...
    {
        if (Spacing != null) {
            child.SetIfDefault(FrameworkElement.MarginProperty, Spacing.Value);
        }
    ...
    public void PerformLayout()
    {
        ...
        if (_shouldReindex || (IsAutoIndexing &&
            ...
            foreach (UIElement child in Children) {
                ...
                if (Spacing != null) {
                    DependencyHelpers.SetIfDefault(child, FrameworkElement.MarginProperty, Spacing.Value);
                }
                if (ChildHorizontalAlignment != null) {
    ...
    public static readonly DependencyProperty SpacingProperty =
        DependencyProperty.Register("Spacing", typeof(Thickness?), typeof(AutoGrid),
            new FrameworkPropertyMetadata((Thickness?)null,
                FrameworkPropertyMetadataOptions.AffectsMeasure | FrameworkPropertyMetadataOptions.AffectsArrange,
                new PropertyChangedCallback(OnSpacingChanged)));
